...
- Node.js >= 18
- 已安装并认证 [Claude Code CLI](https://docs.anthropic.com/en/docs/claude-code) 和/或 [Codex CLI](https://github.com/openai/codex)
...
这个包会安装两个命令别名：`qbeat`（推荐）和 `quotabeat`。
...
立即执行一次所有可用 provider（Claude Code、Codex）的激活。**不会**修改任何计划。
...
每次 provider kick 调用还会追加一条 JSON Lines 记录到：

```
~/.quota-beat/logs/kick.jsonl
```

每条记录包含 `provider` 字段（`claude` 或 `codex`）、调用是否成功、退出码，以及 stdout/stderr 的简短摘要。
